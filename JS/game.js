let block
let zoneStrings = ["Shrinks Zone Diamonds", "Increase Cross Rotation", "Shrinks Ramp Width", "Decrease Circle Radius", "Straightens Lines", "Increases Pusher Speed", "Increases Convayer Speed", "Widens Channel"]
let suffixes = ['M', 'B', 'T', 'Qd', 'Qn', 'Sx', 'Sp', 'Oc', 'N', 'Dc', 'UDc', 'DDc', 'TDc', 'QaDc', 'QiDc', 'SxDc', 'SpDc', 'OcDc', 'NDc', 'Vi']
...
var zonePrices = [500, 2500, 50000, 750000, 15000000, 150000000, 2500000000, 55000000000]
var text;
...
$(document).ready(function () {
  ...
  $('#saveGame').on('click', function() {
    if ($(this).html() !== 'Saved!') {
      $(this).html('Saved!')
      save()
      setTimeout(function(){ $('#saveGame').html('Save Game') }, 3000);      
    }
  })
...
var gameScene = new Phaser.Class({
  ...
  create: function() {
    ...
    this.input.on('pointerdown', ptr => {
      const scale = .08 * ((tokenUpgrades[3].level - 1) * .5 + 1) 
      let target = this.add.image(ptr.worldX, ptr.worldY, 'circle').setScale(scale).setAlpha(.3);
      let ballCount = 0
      for (let i = balls.length - 1; i >= 0; i--) {
        let ball = balls[i];
        if (Math.abs(ball.x - ptr.worldX) + Math.abs(ball.y - ptr.worldY) < 40 + scale * 250) {   
          ballCount++;
          ball.value = ball.value.mul(tokenUpgrades[4].value.mul(tokenUpgrades[4].valueModifier.pow(tokenUpgrades[4].level - 1)).div(100));
  ...
  update: function() {
    ...
    for (let i = balls.length - 1; i >= 0; i--) {
      ...
      if (ball.y > (ball.stage * 1500 - 70)) {
        score(ball)
        const success = 100 - tokenUpgrades[1].value.plus(tokenUpgrades[1].level - 1).toNumber()
        if (!(Phaser.Math.Between(0, 100) > success && ball.stage < zones.length) || balls.length > 700) {
          ball.destroy();
          balls.splice(i, 1);
        } else {
          ball.stage++;
    ...
    if (this.input.mousePointer.y > 575 || this.input.mousePointer.x > 680 || this.input.mousePointer.y < 15 || this.input.mousePointer.x < 0) {
      pointerdown = false
      isDragging = false
    }
  }
...
function score(ball) {
  const mod = new Decimal(tokenUpgrades[6].value.mul(tokenUpgrades[6].valueModifier.pow(tokenUpgrades[6].level - 1))).div(100)
...
function generateBall(x, y, image, delayFrame, stage, value, level) {
  ...
  const success = 100 - tokenUpgrades[2].value.plus(tokenUpgrades[2].level - 1).toNumber()
...
function generateZone(level = 0) {
  ...
  if (zoneCount < 8) {
    if (lockedContainer) {
      ...
      lockedText = scene.add.text(-75, 11, "", {
        fontFamily: 'Arial',
        fontSize: 16,
        color: '#f61a06',
        lineSpacing: 40
  ...
  if (zoneCount == 1) {
    ...
    scene.add.line(0, 0, 0, 9070, 2000, 9070, 0x833b21, .6);
    spawns[8].enabled = true;
  }
  ...
  if (zone.type == 0) {
    ...
    for (i = 0; i < 10; i++) {
      for (j = 0; j < 8; j++) {
        var shape1 = matter.add.image(0 + i * 68.5, (zone.type * 1500) + 200 + j * 150, 'rectangle', null, {
          isStatic: true
        }).setScale(zone.levels[level]).setAngle(45);
        var shape2 = matter.add.image(34 + i * 68.5, (zone.type * 1500) + 275 + j * 150, 'rectangle', null, {
          isStatic: true
        }).setScale(zone.levels[level]).setAngle(45);
    ...
    for (i = 0; i < 3; i++) {
      var shape1 = matter.add.image(125, (zone.type * 1500) + 200 + (i * 400), 'rectangle', null, {
        isStatic: true
      }).setScale(30, 1).setAngle(10);
      var shape2 = matter.add.image(550, (zone.type * 1500) + 200 + (i * 400), 'rectangle', null, {
        isStatic: true
      }).setScale(30, 1).setAngle(-10);
      var ramp1 = matter.add.image(340, (zone.type * 1500) + 400 + (i * 400), 'rectangle', null, {
        isStatic: true
      }).setScale(40, 1).setAngle(0);
      var ramp2 = matter.add.image(340, (zone.type * 1500) + 400 + (i * 400), 'rectangle', null, {
        isStatic: true
    ...
    for (i = 0; i < 6; i++) {
      var shape1 = matter.add.image(240 - zone.levels[level], (zone.type * 1500) + 200 + (i * 200), 'rectangle', null, {
        isStatic: true
      }).setScale(50, 1).setAngle(10);
      var shape2 = matter.add.image(440 + zone.levels[level], (zone.type * 1500) + 300 + (i * 200), 'rectangle', null, {
        isStatic: true
      }).setScale(50, 1).setAngle(-10);
      zone.shapes.push(shape1);
      zone.shapes.push(shape2);
    ...
    zone.levels = [.5, .47, .43, .4, .37, .33, .3, .27, .23, .2]
    for (i = 0; i < 3; i++) {
      var shape1 = matter.add.image(0, (zone.type * 1500) + 275 + (i * 400), 'circle').setScale(zone.levels[level]).setCircle(zone.levels[level] * 250, {
        isStatic: true
      });
      var shape2 = matter.add.image(480, (zone.type * 1500) + 275 + (i * 400), 'circle').setScale(zone.levels[level]).setCircle(zone.levels[level] * 250, {
        isStatic: true
      });
      var shape3 = matter.add.image(240, (zone.type * 1500) + 475 + (i * 400), 'circle').setScale(zone.levels[level]).setCircle(zone.levels[level] * 250, {
        isStatic: true
      });
      var shape4 = matter.add.image(720, (zone.type * 1500) + 475 + (i * 400), 'circle').setScale(zone.levels[level]).setCircle(zone.levels[level] * 250, {
        isStatic: true
      });
      zone.shapes.push(shape1);
      zone.shapes.push(shape2);
    ...
    for (i = 0; i < 12; i++) {
      for (j = 0; j < 8; j++) {
        var shape1 = matter.add.image(10 + i * 60, (zone.type * 1500) + 200 + j * 150, 'rectangle', null, {
          isStatic: true
        }).setScale(7, 1).setAngle(zone.levels[level][0]);
        var shape2 = matter.add.image(40 + i * 60, (zone.type * 1500) + 275 + j * 150, 'rectangle', null, {
          isStatic: true
    ...
    zone.levels = [125, 115, 105, 95, 85, 75, 65, 55, 45, 35]
    for (i = 0; i < 3; i++) {
      var shape1 = matter.add.image(200, (zone.type * 1500) + 400 + (i * 400), 'rectangle', null, {
        isStatic: true
      }).setScale(8).setAngle(45);
      var shape2 = matter.add.image(480, (zone.type * 1500) + 400 + (i * 400), 'rectangle', null, {
        isStatic: true
      }).setScale(8).setAngle(45);
      var shape3 = matter.add.image(340, (zone.type * 1500) + 400 + (i * 400), 'rectangle', null, {
        isStatic: true
      }).setScale(20, 1);
      var shape4 = matter.add.image(200, (zone.type * 1500) + 400 + (i * 400), 'rectangle', null, {
        isStatic: true
      }).setScale(1, 12);
      var shape5 = matter.add.image(480, (zone.type * 1500) + 400 + (i * 400), 'rectangle', null, {
        isStatic: true
      }).setScale(1, 12)
      var shape6 = matter.add.image(150, (zone.type * 1500) + 200 + (i * 400), 'rectangle', null, {
        isStatic: true
      }).setScale(30, 1).setAngle(10)
      var shape7 = matter.add.image(530, (zone.type * 1500) + 200 + (i * 400), 'rectangle', null, {
        isStatic: true
      }).setScale(30, 1).setAngle(-10);
      var tween1 = scene.tweens.add({
        targets: shape4,
    ...
    for (i = 0; i < 3; i++) {
      for (j = -50; j < 600; j = j + 50) {
        var shape = matter.add.image(j, (zone.type * 1500) + 290 + (i * 400) - (j / 5), 'rectangle', null, {
          isStatic: true
      ...
      for (j = 700; j > 100; j = j - 50) {
        var shape = matter.add.image(j, (zone.type * 1500) + 340 + (i * 400) + (j / 5), 'rectangle', null, {
          isStatic: true
        }).setScale(4, 2).setAngle(-10);
        if (j != 150) {
          var tween = scene.tweens.add({
    ...
    for (i = 0; i < 6; i++) {
      var shape1 = matter.add.image(200 - zone.levels[level], (zone.type * 1500) + 200 + (i * 200), 'rectangle', null, {
        isStatic: true
      }).setScale(70, 1).setAngle(10)
      var shape2 = matter.add.image(200 - zone.levels[level], (zone.type * 1500) + 330 + (i * 200), 'rectangle', null, {
        isStatic: true
      }).setScale(70, 1).setAngle(-10)
      zone.shapes.push(shape1);
      zone.shapes.push(shape2);
    ...
    for (i = 0; i < 5; i++) {
      var shape1 = matter.add.image(550 + zone.levels[level], (zone.type * 1500) + 300 + (i * 200), 'rectangle', null, {
        isStatic: true
      }).setScale(70, 1).setAngle(-10)
      var shape2 = matter.add.image(550 + zone.levels[level], (zone.type * 1500) + 430 + (i * 200), 'rectangle', null, {
        isStatic: true
      }).setScale(70, 1).setAngle(10)
      zone.shapes.push(shape1);
      zone.shapes.push(shape2);
...
function upgradeZone(zone) {
  var cost = new Decimal(zonePrices[zone.type]).mul(zone.costModifier.pow(zone.level + 1))
  if (currentScore.lt(cost)) {
    return -1
  }
  zone.level++;
  currentScore = currentScore.minus(cost);
  zone.modifier = .8 + (zone.type * .1) + (zone.level * .05);
  ...
  if (zone.type == 0) {
    ...
    zone.shapes.forEach(function(shape) {
      shape.setScale(zone.levels[zone.level]).setCircle(zone.levels[zone.level] * 250, {
        isStatic: true
      });
    });
  } else if (zone.type == 1) {
    ...
    for (i = 0; i < 3; i++) {
      var ramp1 = matter.add.image(200, (zone.type * 1500) + 400 + (i * 400), 'rectangle', null, {
        isStatic: true
      }).setScale(1, 12);
      var ramp2 = matter.add.image(480, (zone.type * 1500) + 400 + (i * 400), 'rectangle', null, {
        isStatic: true
      }).setScale(1, 12)
      scene.tweens.add({
        targets: ramp1,
  ...
  if ($('#ballShop').is(":visible")) {
    for (var i = 0; i < spawns.length; i++) {
      ...
      if (!zones[spawn.stage - 1]) {
        ...
        const mod = tokenUpgrades[0].value.mul(tokenUpgrades[0].valueModifier.pow(tokenUpgrades[0].level - 1)).div(100)
        if(spawn.level === 0) {
          $('#ball' + i + 'Value').html(displayNumber(mod.mul(spawn.value.mul(spawn.valueModifier.pow(spawn.level)))))
          $('#ball' + i + 'Cooldown').html(displayNumber(spawn.cooldown - (spawn.speedModifier * spawn.level)))
        } else if (spawn.level < 11) {
          $('#ball' + i + 'Value').html(displayNumber(mod.mul(spawn.value.mul(spawn.valueModifier.pow(spawn.level - 1)))) + ' > ' + displayNumber(mod.mul(spawn.value.mul(spawn.valueModifier.pow(spawn.level)))))
          $('#ball' + i + 'Cooldown').html(displayNumber(spawn.cooldown - (spawn.speedModifier * (spawn.level - 1))) + ' > ' + displayNumber(spawn.cooldown - (spawn.speedModifier * spawn.level)))
        } else {
          $('#ball' + i + 'Value').html(displayNumber(mod.mul(spawn.value.mul(spawn.valueModifier.pow(spawn.level - 1)))) + ' > ' + displayNumber(mod.mul(spawn.value.mul(spawn.valueModifier.pow(spawn.level)))))
          $('#ball' + i + 'Cooldown').html(displayNumber(spawn.cooldown - (spawn.speedModifier * 10)) + ' > MAX')
        }
  ...
  if ($('#zoneShop').is(":visible")) {
    for (var i = 0; i < 8; i++) {
      if (zones.length > i) {
        ...
        const cost = new Decimal(zonePrices[i]).mul(zones[i].costModifier.pow(zones[i].level + 1))
        let color = 'black';
        if (cost.gte(currentScore)) {
          color = 'red';
        }
      ...
      if (tokenUpgrades.length > i) {
        ...
        if( i === 1 || i === 2 || i === 5) {
          if (tokenUpgrade.level !== tokenUpgrade.maxLevel) {
            $('#token' + i + 'Value').html(displayNumber(tokenUpgrade.value.plus(tokenUpgrade.level - 1)) + '% > ' + displayNumber(tokenUpgrade.value.plus(tokenUpgrade.level)) + '%')
            const cost = tokenUpgrade.cost.mul(tokenUpgrade.costModifier.pow(tokenUpgrade.level + 1));
            let color = 'black';
            if (cost.gte(tokens)) {
              color = 'red';
            }
            ...
            $('#token' + i + 'Value').html(displayNumber(tokenUpgrade.value.plus(tokenUpgrade.level - 1)) + '%')
            $('#token' + i + 'Cost').html('MAX')
          }
          $('#token' + i + 'Level').html((tokenUpgrade.level - 1) + '/' + (tokenUpgrade.maxLevel - 1))
        } else if( i === 3) {
          if (tokenUpgrade.level !== tokenUpgrade.maxLevel) {
            $('#token' + i + 'Value').html(displayNumber(tokenUpgrade.value.plus((tokenUpgrade.level - 1) * 50)) + '% > ' + displayNumber(tokenUpgrade.value.plus(tokenUpgrade.level * 50)) + '%')
            const cost = tokenUpgrade.cost.mul(tokenUpgrade.costModifier.pow(tokenUpgrade.level + 1));
            let color = 'black';
            if (cost.gte(tokens)) {
              color = 'red';
            ...
            $('#token' + i + 'Value').html(displayNumber(tokenUpgrade.value.plus((tokenUpgrade.level - 1) * 50)) + '%')
            $('#token' + i + 'Cost').html('MAX')
          }
          $('#token' + i + 'Level').html((tokenUpgrade.level - 1) + '/' + (tokenUpgrade.maxLevel - 1))
        } else {
          $('#token' + i + 'Value').html(displayNumber(tokenUpgrade.value.mul(tokenUpgrade.valueModifier.pow(tokenUpgrade.level - 1))) + '% > ' + displayNumber(tokenUpgrade.value.mul(tokenUpgrade.valueModifier.pow(tokenUpgrade.level))) + '%')
          const cost = tokenUpgrade.cost.mul(tokenUpgrade.costModifier.pow(tokenUpgrade.level + 1));
          let color = 'black';
          if (cost.gte(tokens)) {
            color = 'red';
    ...
    if (save) {
      ...
      if (save.tokenUpgrades && save.tokenUpgrades.length > 0) {
        ...
        _.each(tokenUpgrades, function(tokenUpgrade, index) {
          tokenUpgrade.valueModifier = new Decimal(tokenTemplate[index].valueModifier || 0)
          tokenUpgrade.value = new Decimal(tokenTemplate[index].value || 0);
          tokenUpgrade.costModifier = new Decimal(tokenTemplate[index].costModifier || 0);
...
function calcuateTokens() {
  ...
  const mod = new Decimal(tokenUpgrades[7].value.mul(tokenUpgrades[7].valueModifier.pow(tokenUpgrades[7].level - 1))).div(100)
